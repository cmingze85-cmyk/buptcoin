# gui_complete.py - 完整功能GUI，实现CLI所有功能的可视化
import sys
import threading
import time
...
class UserAuthDialog(QDialog):
    ...
    def init_ui(self):
        ...
        if DATABASE_AVAILABLE and db and hasattr(db, 'is_connected') and db.is_connected:
            ...
            db_status = QLabel("⚠️ 使用内存模式（数据不会保存）")
    ...
    def do_login(self):
        ...
        if not username or not password:
            QMessageBox.warning(self, "输入错误", "请填写用户名和密码")
            return
    ...
    def do_register(self):
        ...
        if len(username) < 6 or len(username) > 20:
            QMessageBox.warning(self, "输入错误", "用户名长度应在6-20个字符之间")
            return
        ...
        if password != confirm:
            QMessageBox.warning(self, "输入错误", "两次密码输入不一致")
            return
        ...
        if user_id:
            ...
            if user:
                ...
                QMessageBox.critical(self, "错误", "用户创建成功但获取信息失败")
        else:
            QMessageBox.critical(self, "注册失败", "用户名可能已存在")
...
class BlockchainGUIComplete(QMainWindow):
    """完整功能GUI - 实现所有CLI功能"""
    ...
    def init_ui(self):
        """初始化界面"""
        self.setWindowTitle('💰 BuptCoin - 完整功能版 v4.0')
    ...
    def create_main_tabs(self, layout):
        ...
        tabs.addTab(self.create_vote_tab(), "🗳️ 投票")
    ...
    def create_transaction_tab(self) -> QWidget:
        ...
        self.tx_type_combo.currentTextChanged.connect(self.on_tx_type_changed)
        grid.addWidget(self.tx_type_combo, 3, 1)
        
        grid.addWidget(QLabel("备注/数据:"), 4, 0)
        self.tx_data_edit = QLineEdit()
        self.tx_data_edit.setPlaceholderText("可选")
    ...
    def on_tx_type_changed(self, tx_type):
        """交易类型变更"""
        if tx_type == "stake":
            self.tx_data_edit.setPlaceholderText("质押周期（如：30天）")
        elif tx_type == "vote":
            self.tx_data_edit.setPlaceholderText("候选人名称")
        elif tx_type == "contract":
            self.tx_data_edit.setPlaceholderText("合约调用数据")
        else:
            self.tx_data_edit.setPlaceholderText("备注")
    ...
    def send_transaction(self):
        ...
        try:
            ...
            # 根据交易类型处理
            if tx_type == "stake":
                receiver = sender  # 质押给自己
                data = f"质押周期: {data if data else '30天'}"
            elif tx_type == "vote":
                receiver = "vote_pool"
                data = f"投票给: {data if data else '候选人A'}"
            
            # 检查余额
            balance = self.blockchain.get_balance(sender)
            if balance < amount + self.blockchain.transaction_fee:
                QMessageBox.warning(self, "余额不足", 
                    f"需要: {amount + self.blockchain.transaction_fee:.8f}\n余额: {balance:.8f}")
                return
            ...
            if self.blockchain.add_transaction(tx):
                QMessageBox.information(self, "成功", "交易已提交！")
                self.update_all_displays()
            else:
    ...
    def on_mining_finished(self, success, msg):
        ...
        if success:
            ...
            QMessageBox.information(self, "挖矿成功", msg)
        else:
        ...
        for block in self.blockchain.chain:
            for tx in block.transactions:
                if tx.transaction_type == "stake":
                    addr = tx.sender
                    if addr in stake_amounts:
                        stake_amounts[addr] += tx.amount
                    else:
                        stake_amounts[addr] = tx.amount
    ...
    def update_vote_results(self):
        ...
        for block in self.blockchain.chain:
            for tx in block.transactions:
                if tx.transaction_type == "vote" and "投票给:" in tx.data:
                    candidate = tx.data.split("投票给:")[1].strip()
                    if candidate in votes:
                        votes[candidate] += tx.amount
                    else:
                        votes[candidate] = tx.amount
    ...
    def update_all_displays(self):
        ...
        try:
            ...
            if miner:
                ...
                if idx >= 0:
                    self.miner_combo.setCurrentIndex(idx)
    ...
    def create_wallet(self):
        ...
        if ok and name:
            ...
            self.update_all_displays()
            QMessageBox.information(self, "成功", f"钱包 '{name}' 创建成功！")
...
def main():
    ...
    app.setApplicationName("BuptCoin Complete")
    app.setApplicationVersion("4.0")
    
    try:
        gui = BlockchainGUIComplete()
        gui.show()
        sys.exit(app.exec_())
    except Exception as e:
